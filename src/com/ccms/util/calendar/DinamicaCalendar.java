...
public class DinamicaCalendar extends GenericTransaction
{
	...
	public int service(Recordset inputs) throws Throwable
	{
		...
		//set date
		Date calDate = ValidatorUtil.testDate(d, "yy-MM-dd");
		if (calDate==null)
			calDate = new Date();

		//lower bound date
		Date minDate = ValidatorUtil.testDate(d2, "yy-MM-dd");

		//upper bound date
		Date maxDate = ValidatorUtil.testDate(d3, "yy-MM-dd");
		...
		for (int i=c.get(Calendar.YEAR)-70; i<c.get(Calendar.YEAR)+15;i++)
		{
			rsYears.addNew();
		...
		String partialDate = StringUtil.formatDate(calDate, "yyyy-M");
