...
/**
 * @author zc
 * 将某专题下的业务表数据复制到另一专题下，包括页面设置数据
 *
 */
public class CopyTable extends GenericTableManager {

	public int service(Recordset inputParams) throws Throwable {
		
		int rc = super.service(inputParams);
		
		String isCopyForm = inputParams.getString("is_copy_form");
		...
		Recordset rsFields = db.get(queryFields);
		Recordset rsForms = db.get(queryForms);
		
		//获取业务主键
		String table_id = getSeq(inputParams,querySeq);
		db.addBatchCommand(StringUtil.replace(insertTable, "${table_id}", table_id));
