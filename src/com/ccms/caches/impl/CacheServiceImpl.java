...
public final class CacheServiceImpl implements CacheService {
	...
	public DocumentBean loadDocument(int documentId) {
		...
		try {
			...
			Db db = new Db(conn);
			TemplateBean tmp = new TemplateBean(InitializerServlet.getContext());
			...
			if(formId != null){
				...
				FormBean bean_old = factory.getValue(CacheConst.FORM_OLD_CACHE_PREFIX + formId);
				if(bean_old != null){
					factory.replace(CacheConst.FORM_OLD_CACHE_PREFIX + formId, bean_old, bean);
				}else{
					factory.addCache(CacheConst.FORM_OLD_CACHE_PREFIX + formId, bean);
				}
			}
		} catch (Throwable e) {
			log.error("加载文档失败" + e);
