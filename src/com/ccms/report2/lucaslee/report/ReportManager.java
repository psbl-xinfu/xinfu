package com.ccms.report2.lucaslee.report;
...
import com.ccms.report2.lucaslee.report.grouparithmetic.GroupArithmetic;
...
import com.ccms.report2.lucaslee.report.model.TableColumn;
import com.ccms.report2.lucaslee.report.model.TableLine;
		...
		for (int k = 0; k < t.getColCount(); k++) { // 遍历列
			if (k > nonTotal) { // "小计"列不用添加了
				...
				if (isAmonge(k, totalCols)) { // 指定的统计列才进行汇总,非统计列是一个空字符串
					...
					GroupArithmetic arithMe=arith[k].equals("count")?new SumArithmetic():(
							arith[k].equals("sum")?new SumArithmetic():(
								arith[k].equals("avg")?new AverageArithmetic():(
									arith[k].equals("max")?new MaxArithmetic():(
											arith[k].equals("min")?new MinArithmetic():
												new SumArithmetic()))));
		...
		for (int i = colHead.length; i < result.getRowCount(); i++) {
			for (int j = rowHead.length; j < result.getColCount(); j++) {
				...
				if(!"0.0".equals(val)&&!"0".equals(val)){
					result.getCell(i, j).setContent(val);
				}else{
					if("1".equals(isShowZero))
						result.getCell(i, j).setContent("0");
					else
