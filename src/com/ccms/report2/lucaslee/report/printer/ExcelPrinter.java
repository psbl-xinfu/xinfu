package com.ccms.report2.lucaslee.report.printer;
...
import com.ccms.report2.lucaslee.report.CssEngine;
import com.ccms.report2.lucaslee.report.Printer;
import com.ccms.report2.lucaslee.report.ReportException;
import com.ccms.report2.lucaslee.report.model.Rectangle;
...
public class ExcelPrinter implements Printer {
	...
	private HSSFFont hssfFont = null;//只需要创建一次就行了。
	 ...
	 * @throws IOException
	 */
	public void print(Report r, ExcelCss css, OutputStream result,boolean onlyUseCssClassOnDataCell) throws ReportException, IOException {
	...
	/**
	 * 深度克隆一个Font对象。由于HSSFFont不支持clone方法，所以依次复制属性。(createFont 方法只创建一次就行了)
	 * 
	 * @param wb
	 * @param f
	 * @return
	...
	private HSSFFont cloneFont(HSSFWorkbook wb, HSSFFont f) {
		...
		hssfFont.setBoldweight(f.getBoldweight());
		hssfFont.setColor(f.getColor());
		hssfFont.setFontHeight(f.getFontHeight());
		hssfFont.setFontHeightInPoints(f.getFontHeightInPoints());
		hssfFont.setFontName(f.getFontName());
		hssfFont.setItalic(f.getItalic());
		hssfFont.setStrikeout(f.getStrikeout());
		hssfFont.setTypeOffset(f.getTypeOffset());
		hssfFont.setUnderline(f.getUnderline());
		return hssfFont;
	}
	...
	private void print(TableCell tableCell,
		...
		if (Report.CROSS_HEAD_HEAD_TYPE.equals(tableCell.getCssClass())) {
			...
			String value = (String) tableCell.getContent();
			if(value.length() > 0 && value.length() < 10 && isNumeric(value)){
				try{
				...
				try{
					...
					cell.setCellValue(Double.parseDouble(value.replaceAll("%", ""))/100);
					cell.setCellStyle(style);
				}catch(Throwable e){
					cell.setCellValue(value);
				}
	...
	private HSSFCellStyle getCellStyle(TableCell tableCell, boolean haveBorder,
	 ...
	 * @return
	 */
	private HSSFCellStyle getFullCellStyle(TableCell tableCell, boolean haveBorder, HSSFWorkbook wb, HSSFCellStyle style) {
		if (style == null) {
