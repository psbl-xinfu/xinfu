...
public class HTMLPrinter implements Printer {
	...
	private void print(Table t, OutputStream result) throws ReportException, IOException {
		 ...
		 * Author: 马志礼
		 * Date: 2017-03-18
		 * Content: 在这里把原来的表格样式改为Bootstrap样式
		 */
	...
	private void print(TableCell tc, int width, OutputStream result) throws ReportException, IOException {
		if (tc.getIsHidden()) {
			...
			StringBuffer buf = new StringBuffer();
			buf.append("<td rowspan=").append(tc.getRowSpan()).append(" colspan=").append(tc.getColSpan()).append(" ");
			if (tc.getCssClass() != null){
				buf.append(" class='").append(tc.getCssClass()).append("'");
				/**
				 * Author: 马志礼
				 * Date: 2017-03-18
				 * Content: 去除td上面的width属性
				 */
			...
			buf.append(" style='").append(tc.getCssStyle()).append("'").append(getMultiParamStr(tc)).append(">");
			result.write(buf.toString().getBytes(encoding));
