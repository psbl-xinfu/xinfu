...
public class SearchPickup extends GenericTransaction
{
	...
	public int service(Recordset inputs) throws Throwable
	{
		...
		String sqlFileExistsColumn = getConfig().getConfigValue("sql-template-exists-column");
		...
		if(strNamespace==null || "null".equals(strNamespace) || "".equals(strNamespace)){	//如果命名空间为空,则取外键模版
			...
			String sqlExistsColumn = getResource(sqlFileExistsColumn);
			sqlExistsColumn = StringUtil.replace(sqlExistsColumn, "${table}", table);
			sqlExistsColumn = StringUtil.replace(sqlExistsColumn, "${field_code}", "subject_id");
			Recordset rsExistsField = getDb().get(sqlExistsColumn);
			rsExistsField.first();
			String flag = rsExistsField.getString("flag");
			if("1".equals(flag)){
				qBase = StringUtil.replace(qBase, "${result_field}", "subject_id");
				qBase = StringUtil.replace(qBase, "${result}", "1");
			}else{
				qBase = StringUtil.replace(qBase, "${result_field}", "0");
				qBase = StringUtil.replace(qBase, "${result}", "0");
			}
		}
