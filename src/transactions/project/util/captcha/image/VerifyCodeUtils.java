...
public class VerifyCodeUtils {
	...
	public static void outputImage(int w, int h, OutputStream os, String code)
		...
		Color fontColor = getRandColor(100, 160);
		// 绘制粗体贯穿干扰线
		g2.setStroke(new BasicStroke(10f));
		g2.setColor(fontColor);// 设置线条的颜色
		for (int i = 0; i < 1; i++) {
			int x = random.nextInt(w/5);
			int y = random.nextInt(h/4) + h/4;
			int xl = random.nextInt(w/5) + 4*w/5;
			int yl = random.nextInt(h/4) + 2*h/4;
