SELECT 
concat( '<input type="checkbox" class="custcomm" name="custcomm" value="',f.tuid,'" code1="', 
	(select status from cc_feedback_follow where feedback_id = f.tuid and org_id = ${def:org} order by created desc limit 1),'">') as application_id,
	f.tuid,
