(SELECT
	concat('<label class="am-checkbox"><input type="checkbox" code2="1" data-am-ucheck name="datalist" code="',d.code,'" value="',r.code,'','" ></label>') as application,
		'否' as ispublic,
...
--卡类型
and	(case when ${fld:f_cardtype} is null then 1=1 else d.cardtype::integer = ${fld:f_cardtype} end)
--卡状态
AND (
  	case when ${fld:f_status} is null then 1=1
  	when ${fld:f_status} = 1 then d.status = 1
  	when ${fld:f_status} = 2 then d.status = 2
  	when ${fld:f_status} = 3 then d.status = 3
  	when ${fld:f_status} = 4 then d.status = 4
  	when ${fld:f_status} = 5 then d.status = 5
  	when ${fld:f_status} = 6 then d.status = 6
  	when ${fld:f_status} = 0 then d.status = 0
  	else true end
  )
--分配次数
and
	(case when ${fld:f_distributioncount} is null  then 1=1 else 
		(select count(1) from cc_mcchange m 
		where m.customercode = r.code and m.org_id = r.org_id)<${fld:f_distributioncount}::int
	 end)
 ...
 (
select
	concat('<label class="am-checkbox"><input type="checkbox" code2="2" data-am-ucheck name="datalist" value="',r.code,'','" ></label>') as application,
	'是' as ispublic,
	(select  file_path from t_attachment_files where  t_attachment_files.pk_value =r.code and org_id= ${def:org} order by tuid desc limit 1) as img_url,--图片
	r.name as mc_name,
	r.code as mc_code,--会员编号
