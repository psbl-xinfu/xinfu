SELECT 
	...
	(case when (${fld:period_day}::int-('${def:date}'::date -(select created from cc_ptchange where customercode = c.code and org_id = ${def:org} order by created desc limit 1)::date))<1 then 0 
 	else (${fld:period_day}::int-('${def:date}'::date -(select created from cc_ptchange where customercode = c.code and org_id = ${def:org} order by created desc limit 1)::date)) end) as num_days, --保护期
