select
	...
	(select (case when lefttime is null then (select cabinettempcode from cc_cabinettemp where tuid::varchar = cc_inleft.cabinettempcode and org_id = ${fld:unionorgid}) else '' end)
		from cc_inleft where indate = '${def:date}'::date and cardcode = card.code and org_id = ${fld:unionorgid} order by intime desc LIMIT 1) as cabinettempcode,
