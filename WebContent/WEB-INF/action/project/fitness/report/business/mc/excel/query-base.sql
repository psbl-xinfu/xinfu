...
inner join (
	select c.salemember as userlogin, count(1) as cardnum, 
 			...
 			0 as xufee
 		from cc_contract c 
 		where c.createdate >=  ${fld:daochu_start_date} and c.createdate <=  ${fld:daochu_end_date}
 		...
 		and  salemember=(case when  ${fld:daochu_skill_name} is null  then salemember else ${fld:daochu_skill_name} end )
 		...
 		and  salemember1=(case when  ${fld:daochu_skill_name} is null  then salemember1 else ${fld:daochu_skill_name} end )
...
where h.status = 1 and h.userlogin = (case when ${fld:daochu_skill_name} is null then h.userlogin else ${fld:daochu_skill_name} end ) 
and exists(
	select 1 from hr_skill k 
	inner join hr_staff_skill fk on k.skill_id = fk.skill_id 
