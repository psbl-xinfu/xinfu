...
and (CASE when ${fld:salename_query} is not null then (case when ${fld:salename_query}='3' then 
	 	(case when ${fld:yewuleixing} is not null then (case when ${fld:yewuleixing}='1001' then 1=2 else 1=1 end) else 1=1 end) else 1=1
	 end) else 1=1 end)
...
where s.created::date >= ${fld:_start_date}
	 and s.created::date <= ${fld:_end_date}
	 and money !=0 and s.org_id = ${def:org}
	 and (CASE when ${fld:salename_query} is not null then (case when ${fld:salename_query}='3' then 
	 	(case when ${fld:yewuleixing} is not null then (case when ${fld:yewuleixing}='1001' then 1=1 else 1=2 end) else 1=1 end) else 1=2
	 end) else 1=1 end)
and (case when ${fld:shouyinyuan} is null then 1=1 else s.collectby=${fld:shouyinyuan} end)
and (case when ${fld:xiaoshouyuan} is null then 1=1 else s.createdby=${fld:xiaoshouyuan} end)
${orderby}
