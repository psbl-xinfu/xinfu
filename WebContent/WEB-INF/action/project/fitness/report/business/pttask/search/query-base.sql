...
where staff.status = 1 and staff.org_id = ${def:org}
 	and (case when ${fld:skill_name} is null then 1=1 else staff.userlogin=${fld:skill_name} end)
and exists(
...
FULL JOIN(
	...
	and (case when ${fld:end_date} is null then 1=1 else tr.created::date<=${fld:end_date} end)
 	and (case when ${fld:skill_name} is null then 1=1 else tr.createdby=${fld:skill_name} end)
	and exists(
		select 1 from hr_skill k 
		inner join hr_staff_skill fk on k.skill_id = fk.skill_id 
