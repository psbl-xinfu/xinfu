...
inner join (select user_id from hr_team_staff 
	...
	select team_id from hr_team where org_id = ${def:org} and
		skill_scope in ('1','2','4','8')) team on team.team_id = hr_team_staff.team_id
...
FULL JOIN(
	...
	and exists(
		...
		where team.skill_scope in ('1','2','4','8') and hts.userlogin = ts.userlogin and team.org_id = ts.org_id
	)
...
FULL JOIN(
	...
	and exists(
		select 1 from hr_team team 
		inner join hr_team_staff hts on team.team_id = hts.team_id 
		where team.skill_scope in ('1','2','4','8') and hts.userlogin = g.initmc and team.org_id = g.org_id
	) GROUP BY g.initmc
...
FULL JOIN(
	...
	and exists(
		select 1 from hr_team team 
		inner join hr_team_staff hts on team.team_id = hts.team_id 
		where team.skill_scope in ('1','2','4','8') and hts.userlogin = c.createdby and team.org_id = c.org_id
...
FULL JOIN(
	...
	and exists(
		select 1 from hr_team team 
		inner join hr_team_staff hts on team.team_id = hts.team_id 
		where team.skill_scope in ('1','2','4','8') and hts.userlogin = gp.createdby and team.org_id = gp.org_id
...
FULL JOIN(
	...
	and exists(
		select 1 from hr_team team 
		inner join hr_team_staff hts on team.team_id = hts.team_id 
		where team.skill_scope in ('1','2','4','8') and hts.userlogin = gv.mc and team.org_id = gv.org_id
	) GROUP BY gv.mc
...
FULL JOIN(
	...
	and exists(
		select 1 from hr_team team 
		inner join hr_team_staff hts on team.team_id = hts.team_id 
		where team.skill_scope in ('1','2','4','8') and hts.userlogin = c.salemember and team.org_id = c.org_id
 		)
	...
	and exists(
		select 1 from hr_team team 
		inner join hr_team_staff hts on team.team_id = hts.team_id 
		where team.skill_scope in ('1','2','4','8') and hts.userlogin = c.salemember1 and team.org_id = c.org_id
 		)
