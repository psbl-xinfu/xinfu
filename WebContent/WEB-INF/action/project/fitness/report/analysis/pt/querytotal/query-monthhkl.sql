select 
	sum(freenum)+sum(feenum) as monthhaokelv--当前耗课率
from
(select 
...
from (
	...
	and (case when ${fld:fdate} is null then 1=1 else p.created::date>=(${fld:fdate} - interval '1 month') end)
	and (case when ${fld:tdate} is null then 1=1 else p.created::date<=(${fld:tdate} - interval '1 month') end)
	and exists(
...
full join (
	...
	and (case when ${fld:fdate} is null then 1=1 else c.createdate>=(${fld:fdate} - interval '1 month') end)
	and (case when ${fld:tdate} is null then 1=1 else c.createdate<=(${fld:tdate} - interval '1 month') end)
	and exists(
 			select 1 from hr_skill k 
 			inner join hr_staff_skill fk on k.skill_id = fk.skill_id 
