(select
	...
	(case when (${fld:period_day}::int-('${def:date}'::date -(select created from cc_mcchange where guestcode = g.code order by created desc limit 1)::date))<1 then 0 
 	else (${fld:period_day}::int-('${def:date}'::date -(select created from cc_mcchange where guestcode = g.code order by created desc limit 1)::date)) end) as num_days,
	g.created,
...
(
select
	concat('<input type="checkbox" code2="2" class="duoxuan" name="danxuan" value="', g.code, '">') as application_id,
	g.code as vc_code,
	g.name as vc_name,
	(case g.sex when 0 then '女' when 1 then '男' when 2 then '未知' else '' end) as i_sex,
	g.age as i_age,
