SELECT 
	COALESCE(g.allclass_target,0) AS allclass_target
	,COALESCE(g.unpayclass_target,0) AS unpayclass_target
	,COALESCE(g.call_pt_target,0) AS call_pt_target
...
AND g.org_id = ${def:org} AND s.userlogin = '${def:user}'
