with ptdata as (
				select concat(${fld:preparedate}, ':', ${fld:preparetime})::time + concat(d.times,' minutes')::interval as endtime,
...
select count(1) as workdate from dual 
where exists(
...
(case when 
	...
	and
	(
		(p.preparetime > concat(${fld:preparedate}, ':', ${fld:preparetime})::time and p.preparetime <= (select endtime from ptdata) and p.endtime >= (select endtime from ptdata))
		or (p.preparetime <= concat(${fld:preparedate}, ':', ${fld:preparetime})::time and p.endtime >= (select endtime from ptdata))
		or (p.preparetime < (select endtime from ptdata) and p.preparetime <=concat(${fld:preparedate}, ':', ${fld:preparetime})::time and p.endtime >= concat(${fld:preparedate}, ':', ${fld:preparetime})::time)
	) 
	and 
