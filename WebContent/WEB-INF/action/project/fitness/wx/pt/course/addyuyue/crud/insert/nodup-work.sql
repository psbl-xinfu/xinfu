SELECT 1 FROM dual 
WHERE not (${fld:preparetime}::time>=(SELECT
...
and 
	(${fld:preparetime}::time+ ((select times from cc_ptdef where code = (select ptlevelcode from cc_ptrest where code = ${fld:ptrestcode} and org_id = ${def:org}) and org_id = ${def:org})||' minutes')::interval)<= (SELECT
   config.param_value::time
FROM
	cc_config config
WHERE
