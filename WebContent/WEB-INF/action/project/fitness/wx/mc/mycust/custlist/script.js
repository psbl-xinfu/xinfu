        	...
        	<list>
				str+='<li class="list-li">'
		        	...
		        	str+='        <img src="'+contextPath+'/images/icon_head.png" class="touxiangpic fl">'
		            		...
		            		str+=' <div class="content fl" " onclick="getinfo(\'${fld:guestcode}\',\'${fld:thecode}\')">'
		            			str+='<p class="name" style="width:120%;"><label style="width:40%;white-space:nowrap;text-overflow:ellipsis;overflow:hidden;" title="${fld:vc_name}">${fld:vc_name}</label><span>${fld:i_sex}&ensp;${fld:vc_mobile}</span></p>'
		            	        	str+=' <span class="thespan">${fld:cc_position}</span><p class="time">最近跟进：${fld:created}</p>'
		        	str+='   </div>'
		        	str+=' <a href="tel:${fld:vc_mobile}"> <img src="'+contextPath+'/js/project/fitness/wx/image/tel.png" class="telpic fr"></a>'
		        	str+=' </div>'
		        	str+='<div class="thebtn" id="thecomm" onclick="getcomm(\'${fld:guestcode}\',\'${fld:thecode}\')">跟进</div>'
		        str+='</li>'		
	        	</list>
			str+='<ul>'
		str+='</section>'
