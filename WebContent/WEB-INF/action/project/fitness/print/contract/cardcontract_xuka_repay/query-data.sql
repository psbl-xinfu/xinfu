...
SELECT 
	...
	,'' AS vc_rank
	,c.mc as vc_mc
	,get_arr_value(t.relatedetail,5) AS c_startdate /**20170110*/
	,d.code as vc_cardcode
	,get_arr_value(t.relatedetail,6) AS c_enddate /**20170110*/
	,'' AS vc_company /**20170110*/
	,'' AS vc_validdays /**20170110*/
	,concat(t.remark,dt.remark) AS vc_remark /**20170110*/
	,CASE d.starttype WHEN 0 THEN '接原卡启用' WHEN 1 THEN '原卡过期后初次训练启用' WHEN 2 THEN '固定时间启用' END AS vc_starttype
...
LEFT JOIN cc_cardtype dt ON get_arr_value(t.relatedetail,3) = dt.code and d.org_id = dt.org_id
