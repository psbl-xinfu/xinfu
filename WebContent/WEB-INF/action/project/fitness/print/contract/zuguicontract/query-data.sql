...
SELECT 
	...
	,c.card AS vc_id --20170110
	,'' AS vc_contact --20170110
	,t.salemember AS vc_salemember --20170110
	,c.addr AS vc_addr --20170110
	,c.wx
	,c.qq
	,c.name as vc_name
	...
	,d.count AS vc_cardcount --次卡次数 20170110
	,'' AS vc_company
   ,(
	...
	,(
		CASE WHEN t.type = 1 OR t.type = 12 THEN (
			 (CASE WHEN get_arr_value(t.relatedetail, 5) = '' THEN '0.00' ELSE get_arr_value(t.relatedetail, 5) END)::numeric(10,2)
		) ELSE 0.00::numeric(10,2) END
	) AS f_deposit	-- 租柜合同押金20170110 */
	,CASE WHEN t.type = 1 OR t.type = 12 THEN (get_arr_value(t.relatedetail, 1)) END AS vc_cabinetcode	--储物柜编号 20170110*/
	,CASE WHEN t.type = 1 OR t.type = 12 THEN (get_arr_value(t.relatedetail, 3)) END AS c_startdate	--租柜开始日期 20170110*/
	,CASE WHEN t.type = 1 OR t.type = 12 THEN (get_arr_value(t.relatedetail, 4)) END AS c_enddate	-- 租柜结束日期 20170110*/
	...
	)::numeric(10,2) as f_contract_money
	,(t.normalmoney - t.factmoney)::numeric(10,2) AS f_leftmoney
	,t.remark AS vc_remark /*  备注 20170110*/
	,(
		SELECT a.file_path FROM t_attachment_files a 
		WHERE a.table_code = 'c_customer' AND a.pk_value = c.code 
...
LEFT JOIN cc_card d ON c.code = d.customercode AND d.isgoon = 0 and d.status != 0 and c.org_id = d.org_id
