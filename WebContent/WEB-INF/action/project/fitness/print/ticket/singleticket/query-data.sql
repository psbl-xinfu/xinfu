...
select
	...
	sl.money as f_money,--金额
	string_agg((sl.name || '<br/>' || sl.itemcode || '      ' ||
   		sl.price::numeric(10,2) || '*'||sl.amount||'       '||sl.money::numeric(10,2)),'<br/>') as gouwu_xiaopiao,
    (select count(1) from cc_singleitemdef sd where sd.code =sl.itemcode and sd.org_id = sl.org_id) as gouwu_jianshu,
	(case sl.paytype when 1 then '现金/银行卡支付' when 2 then '储蓄卡支付' else '' end) as i_paytype,
	(select name from cc_customer cust where cust.code = sl.customercode and cust.org_id = sl.org_id) as vc_name, --姓名
	...
	sl.collectdate as c_adate,----收款时间
