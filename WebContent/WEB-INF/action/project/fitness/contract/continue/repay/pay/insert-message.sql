...
SELECT 
	...
	,1
	,(SELECT tuid FROM cc_msg_template WHERE templatecode = 'HYK_XKHK' AND status = 1 LIMIT 1)	-- remind
	,'${def:user}'
	...
	,(
		...
		FROM cc_msg_template m WHERE m.templatecode = 'HYK_XKHK' AND m.status = 1 LIMIT 1
	)	-- content
	,1
	,(SELECT remind FROM cc_msg_template WHERE templatecode = 'HYK_XKHK' AND status = 1 LIMIT 1)	-- remind
	,{ts '${def:timestamp}'}
	,NULL
	,t.org_id  
FROM cc_contract t 
