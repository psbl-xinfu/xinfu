...
inner join (select ss.user_id from hr_staff_skill ss
		inner join (select skill_id from hr_skill where skill_scope in ('1','2','4','8') and org_id = ${def:org}) skill
...
and exists(
		...
		where sk.user_id = so.user_id and k.skill_scope in ('1','2','4','8')
	) 
