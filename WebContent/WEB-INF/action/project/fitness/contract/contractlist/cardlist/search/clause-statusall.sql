and
		...
		and COALESCE(c.normalmoney, 0)!=(COALESCE(c.factmoney, 0)+COALESCE((select ct.factmoney from cc_contract ct 
			where ct.code = c.relatecode and ct.org_id = c.org_id), 0))  and c.status=2 and c.normalmoney != (case when 
(select ct.factmoney from cc_contract ct 
			where ct.relatecode = c.code and ct.org_id = c.org_id) is null then '0' else (select (ct.factmoney+c.factmoney) as bin from cc_contract ct 
			where ct.relatecode = c.code and ct.org_id = c.org_id)
end)
		when ${fld:statusall}='6' then c.contracttype!=3 --6已还款
			and COALESCE(c.normalmoney, 0)!=(COALESCE(c.factmoney, 0)+COALESCE((select ct.factmoney from cc_contract ct 
			where ct.code = c.relatecode and ct.org_id = c.org_id), 0)) and c.status=2 
