select sum(a.huifang) as huifangsijiao from
(
select 
	count(1) as huifang
from cc_guest guest
...
and exists(
		...
		inner join 
			(select skill_id from hr_skill sk left join cc_target_group tg on sk.skill_id = tg.pk_value and sk.org_id = tg.org_id
