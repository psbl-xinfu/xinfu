SELECT 
	p.created AS c_idate,	-- 日期
	COUNT(1) AS pi_prepare_num	-- P1预约人数
FROM cc_prepare p 
WHERE to_char(p.created, 'yyyy-MM') = ${fld:month} 

--AND EXISTS (
	--SELECT 1 FROM cc_ptrest t WHERE t.cardcode = p.cardcode AND t.pttype = 5 
	--AND t.org_id = ${def:org} AND NOT EXISTS (
		--SELECT 1 FROM cc_ptlog g WHERE t.vcode = g.ptcode AND g.org_id = ${def:org} and g.created < (${fld:month}||'-01 00:00:00')::timestamp 
	--)
--)
