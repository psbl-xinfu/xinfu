SELECT 
	g.created::date AS c_idate,	-- 日期
	COUNT(1) AS pi_prepare_success_num	-- P1出现人数
FROM cc_ptrest p 
INNER JOIN cc_ptlog g ON p.code = g.ptrestcode and p.org_id = g.org_id 
WHERE to_char(g.created, 'yyyy-MM') = ${fld:month} 
AND p.pttype = 5 AND NOT EXISTS (
	SELECT 1 FROM cc_ptlog g WHERE p.code = g.ptrestcode and g.org_id = p.org_id AND g.created < (${fld:month}||'-01 00:00:00')::timestamp 
) 
