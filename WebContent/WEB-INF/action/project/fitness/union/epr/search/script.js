...
datagrid.empty();
<rows>
	var str = '<tr onclick=detail("'+$("#month").val()+'-${fld:day}")>';
	str += '<td class="text-center" nowrap>${fld:day}</td>';	// 日期
...
//查询该登录人下所有店数据   （分开查询，各店显示各店数据）
function detail(date){
	ccms.dialog.open({url : "${def:context}${def:actionroot}/eprdetail/crud?date="+date, height: 600});
}
