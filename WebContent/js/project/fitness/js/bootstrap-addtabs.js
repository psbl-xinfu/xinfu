...
$.fn.addtabs = function (options) {
	...
	$(this).on('click','[role=presentation]',function(){
		...
		obj.find("#"+$(this).attr("id").substring(4)).addClass("active");
		is_do_closing = false;
	});
    ...
    $(options.monitor).on('touchstart', '.menu', function () {
    ...
    _add_tab = function (opts) {
        ...
        if ( !$("#" + id,obj)[0] || opts.istabreplace == true ) {
            ...
            //创建新TAB的title
        	var title = $('<li></li>').attr('role', 'presentation').attr('id', 'tab_' + id).append('<span>'+opts.title+'</span>');   
            //是否允许关闭
            if (opts.close!="false") {
            	title.append($("<i></i>").addClass("close-tab am-icon-close")
                        .attr('aria-controls', id)
                        .attr('role', 'tab')
                        .attr('close', opts.close)
                        .attr('data-toggle', 'tab'));
            }else{
            	title.append($("<i style='display:none;'></i>").addClass("close-tab am-icon-close")
                        .attr('aria-controls', id)
                        .attr('role', 'tab')
                        .attr('close', opts.close)
                        .attr('data-toggle', 'tab'));
           ...
           // obj.find('#tablist').append(title);
        	...
        	if (options.iframeUse && !opts.ajax) {
    			...
    			if( oldurl != newurl ){	// 路径、参数变化时时
    				...
    				obj.find("#tablist").find("#tab_" + tabid).find("a[href=#" + tabid + "]").text(opts.title);
    			}
        	}
		}
        //激活TAB
        $("#tab_" + id,obj).addClass('active');
    ...
    _close = function (id) {
        ...
        if (obj.find("li.active").not(".dropdown").attr('id') == "tab_" + id) {
        	...
        	if(tabCurrent.prev().size()>0){
        		tabCurrent.prev().addClass('active');
        		var code=tabCurrent.prev().find("i").attr("aria-controls");
        		$("#"+code).addClass("active");
        		tabCurrent.prev().find("[role=presentation]").click();	/*IE中iframe会变短，点击一下就恢复正常*/
        	}else{
        		var tabActive=$("#tablist",obj).find(">li").not(".tabManage,.dropdown").last();
        		tabActive.addClass('active');
        		var code=tabActive.find("i").attr("aria-controls");
        		$("#"+code).addClass("active");
        		tabActive.find("[role=presentation]").click(); /*IE中iframe会变短，点击一下就恢复正常*/
        	}
        }
        //关闭TAB
        $("#tab_" + id).remove();
    ...
    _drop = function () {
        element=obj.find('#tablist').not('#menuTab');/*排除下位式菜单内容*/
        //创建下拉标签
        ...
        element.find('>li')
                ...
                .each(function () {
                	var offsetRight=$("#tablist",obj).width()-this.offsetLeft-this.offsetWidth+$("#tablist",obj)[0].offsetLeft;
                    if (/**this.offsetTop > 1 || */offsetRight <= 110) {
    ...
    $("#closeCurrent").unbind().on("click", function () { 
    	var obj=$('#tablist').find("li.active").not(".dropdown");
    	obj=obj.find("i[role='tab']");
    	if(obj!=undefined){
