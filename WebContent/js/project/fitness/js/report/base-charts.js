...
function initPieCharts(id, obj, magicType){
	...
	require(
		...
		function (ec) {
			...
			if( undefined != obj.seriesDataArrOther && obj.seriesDataArrOther.length > 0 ){
				var seriesDataOther = [];
				for( var i = 0; i < obj.seriesDataArrOther.length; i++ ){
					seriesDataOther[i] = {
						name: obj.legendDataOther[i],
						value: obj.seriesDataArrOther[i]
					};
					option.legend.data.push(obj.legendDataOther[i]);
				}
				
				option.series.push({
					name: obj.othertitlename,
					type:'pie',
					radius: ['70%', '90%'],
					avoidLabelOverlap: false,
					label: {
	...
	obthis.updateOption = function(seriesData){
		...
		for(var i = 0; i < seriesData.length; i++){
			...
			if( seriesJson["value"] > 0 ){
				dataArr[dataArr.length] = {value: seriesJson["value"], name: seriesJson["name"], num: seriesJson["num"]};
			}
		}
		...
		var option = {
			...
			tooltip: {
				...
				formatter:function(data){ 
					var relVal = data[0]+"<br/>"+data[1]+"（"+data[5].num+"）："+data[2]+"%"; 
					return relVal; 
				} 
			},
			legend: {
...
function pieMultipleCharts(id){
	...
	obthis.updateOption = function(seriesArr){
		...
		for( var i = 0; i < seriesArr.length; i++ ){
			seriesArr[i]["itemStyle"] = labelBottom;
		}
		var option = {
			tooltip: {
				trigger: 'item',
...
function pieCharts(id){
	...
	obthis.updateOption = function(seriesData){
		...
		for( var i = 0; i < seriesData.length; i++ ){
			...
			seriesData[i]["itemStyle"] = labelBottom;
		}
			   ...
			   series : [
			        {
			            ...
			            data : [
			            	{name:'other', value:parseInt(100-Number(seriesData[0]["value"])), itemStyle : labelBottom},
			            	{name:seriesData[0]["name"], value:seriesData[0]["value"],itemStyle : labelTop}
			            ]
			        },
			    ]
			};
