...
dTree.prototype.node = function(node, nodeId) {
	...
	if (this.config.check) {
		if ((this.showBoxLevel != 'no')
				&& (this.getNodeLevel(nodeId) > this.showBoxLevel || !node._hc)) {// 符合层级要求的显示checkbox
			if (node._checked) {
				str += '<input type="checkbox" name="' + node.id + '" id="c'
						+ this.obj + nodeId + '" onclick="javascript:'
						+ this.obj + '.cc(' + nodeId + ')" checked/>';
			} else {
				str += '<input type="checkbox" name="' + node.id + '" id="c'
						+ this.obj + nodeId + '" onclick="javascript:'
						+ this.obj + '.cc(' + nodeId + ')"/>';
		...
		if ((this.showBoxOnlyLeaf != 'no')
				&& (this.showBoxOnlyLeaf == 'leaf' && !node._hc)) {// leaf只有叶子节点显示checkbox,all都显示
			if (node._checked) {
				str += '<input type="checkbox" name="' + node.id + '" id="c'
						+ this.obj + nodeId + '" onclick="javascript:'
						+ this.obj + '.cc(' + nodeId + ')" checked/>';
