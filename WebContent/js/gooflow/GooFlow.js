...
//初始化区域图的对象
	...
	if(property.haveHead){
		...
		this.onBtnEditClick=null;//编辑时按钮
		this.onBtnDeleteClick=null;//删除时按钮
		this.onBtnPublishClick=null;//发布时按钮
		...
		this.$head.on("click",{inthis:this},function(e){
			...
			switch($(tar).attr("class")){
				...
				case "ico_delete":	if(This.onBtnDeleteClick!=null)	This.onBtnDeleteClick();break;
				case "ico_publish":	if(This.onBtnPublishClick!=null)	This.onBtnPublishClick();break;
	...
	if(this.$editable){
	  ...
	  this.onItemAdd=null;
	  //绑定节点或者线双击时处理的事件
	  //格式function(id, type, json):id是单元的唯一标识ID,type是单元的种类,有"node","line"两种值,json即addNode或addLine方法的第二个传参json.
...
GooFlow.prototype={
	...
	initDraw:function(id,width,height){
		...
		if(this.$editable){
			...
			$(this.$draw).delegate(tmpClk,"dblclick",{inthis:this},function(e){
				...
				var This=e.data.inthis;
				//判断如果存在线的话则调用双击事件
				if(This.onItemDblClick!=null && This.$lineData[this.id]){
					This.onItemDblClick(this.id,"line",This.$lineData[this.id]);
				}else{
					if(GooFlow.prototype.useSVG!=""){
	...
	addNode:function(id,json){
		...
		//if(json.type!="start"&&json.type!="end"&&json.type!="blank"){
		if(json.type.indexOf(" round")<0 &&json.type!="blank"){
			...
			this.$nodeDom[id]=$("<div class='GooFlow_item item_round"+mark+"' id='"+id+"' style='top:"+json.top+"px;left:"+json.left+"px'><table cellspacing='0'><tr><td class='ico'><b class='ico_"+json.type+"'></b></td></tr></table><div  style='display:none'><div class='rs_close'></div></div><div class='span'>"+(json.type=="blank"?"":json.name)+"</div></div>");
		}
	...
	initWorkForNode:function(){
		...
		this.$workArea.delegate(".GooFlow_item > .span","dblclick",{inthis:this},function(e){
			...
			//判断如果是节点存在的话则弹出窗口
			if(This.onItemDblClick!=null && This.$nodeData[id]){
				This.onItemDblClick(id,"node",This.$nodeData[id]);
			}else{
				var t=getElCoordinate(This.$workArea[0]);
	...
	resizeNode:function(id,width,height){
		...
		//升级到0.5版本后的bug解决（无法给首节点和结束节点调整长宽）
	...
	loadData:function(data){
		...
		this.setTitle(this.$flowData.wfm_name);
		if(data.initNum) this.$max=data.initNum;
		if(this.$flowData.max) this.$max=this.$flowData.max;
	...
	exportData:function(){
		//导出之前保存序列值
		this.$flowData.max = this.$max+1;
		return {flow:this.$flowData,nodes:this.$nodeData,lines:this.$lineData,areas:this.$areaData};
	},
...
///////////以下为有关画线的方法
	...
	addLine:function(id,json){
		...
		if(!res)	return;
		//直接用传过来的json数据赋值，避免数据丢失
		this.$lineData[id]=json;
		if(!json.type){
			this.$lineData[id].type="sl";//默认为直线
		}
		if(!json.mark)	this.$lineData[id].marked=false;
