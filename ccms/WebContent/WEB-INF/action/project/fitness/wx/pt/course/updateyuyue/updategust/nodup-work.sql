...
and 
	(concat(${fld:hour}, ':', ${fld:minute})::time+ ((select times from cc_ptdef where code = (select ptlevelcode from cc_ptrest where code = ${fld:code} and org_id = ${def:org}) and org_id = ${def:org})||' minutes')::interval)<= (SELECT
   config.param_value::time
FROM
	cc_config config
WHERE
