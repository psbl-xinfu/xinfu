...
where org.org_id in (SELECT g.org_id FROM hr_org g WHERE (case when (select org_id from hr_staff where weixin_lastlogin = ${fld:weixin_userid}) is null 
			then 1=1 else g.org_id = (select org_id from hr_staff where weixin_lastlogin = ${fld:weixin_userid}) end) 
			UNION 
