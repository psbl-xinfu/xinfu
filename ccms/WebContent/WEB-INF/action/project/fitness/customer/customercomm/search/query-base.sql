...
and (case when ${fld:custtype} is null then 1=1 else '2'=${fld:custtype} end)
and (case when ${fld:specialtype} is null then 1=1 else '0'=${fld:specialtype} end)
--回访状态
and (case when ${fld:f_returntype} is null then 1=1 
	when ${fld:f_returntype}='0' then 
	exists(
...
--本月未回访
and (case when ${fld:thismonthtypeone} is null then 1=1 else exists(
			select 1 from cc_return_log where org_id = ${def:org}
			and custtype=0 and (coMMcode is null or coMMcode='')
			and to_char(created, 'yyyy-MM')=to_char('${def:date}'::date, 'yyyy-MM')
			and pk_value = guest.code
...
--本月分配
and (case when ${fld:thismonthtypetwo} is null then 1=1 else exists(
				select 1 from cc_return_log where org_id = ${def:org}
				and custtype=0
				and to_char(created, 'yyyy-MM')=to_char('${def:date}'::date, 'yyyy-MM')
				and pk_value = guest.code
...
/*-------------------------筛选-------------------------------------*/
--性别
and (case when ${fld:f_sex} is null then 1=1 else exists(
				select 1 from cc_guest where org_id = ${def:org}
				and guest.sex=${fld:f_sex}
			) end)

--会籍
and (case when ${fld:f_mc} is null then 1=1 else exists(
				select 1 from cc_guest where org_id = ${def:org}
				and guest.mc=${fld:f_mc}
			) end)

--私教
and (case when ${fld:f_pt} is null then 1=1 else false end)
			
--沟通阶段
and (case when ${fld:f_stage} is null then 1=1 else exists(
				select 1 from cc_comm where cust_type=0
				and stage=${fld:f_stage}
				and guestcode=guest.code
			) end)

--年龄段
and (case when ${fld:f_age} is null then 1=1 else exists(
				select 1 from cc_guest where org_id = ${def:org}
				and guest.age=${fld:f_age}
			) end)	
--通话次数
and (case when ${fld:f_calltimes} is null then 1=1 else exists(
				select 1 from cc_comm where cust_type=0
				and (select count(customercode)  from cc_comm where guestcode=guest.code)<${fld:f_calltimes}
			) end)	
			
--未续费原因
and (case when ${fld:f_leave} is null then 1=1 else false end)
			
--未跟进天数
and (case when ${fld:f_unfollow} is null then 1=1 else exists(
				select 1 from cc_comm where cust_type=0
...
--会籍关注度
and (case when ${fld:f_mc_status} is null then 1=1 else exists(
				select 1 from cc_comm where cust_type=0
				and  level=${fld:f_mc_status}
				and  operatortype=0 and guestcode = guest.code
			) end)	
			
--私教关注度
and (case when ${fld:f_pt_status} is null then 1=1 else exists(
				select 1 from cc_comm where cust_type=0
				and  level=${fld:f_pt_status}
				and  operatortype=1 and guestcode = guest.code
			) end)	
--生日
and (case when ${fld:f_startmonth} is null then 1=1 else guest.birth::int>=${fld:f_startmonth}::int end)
and (case when ${fld:f_startday} is null then 1=1 else guest.birthday::int>=${fld:f_startday}::int end)
and (case when ${fld:f_endmonth} is null then 1=1 else guest.birth::int<=${fld:f_endmonth}::int end)
and (case when ${fld:f_endday} is null then 1=1 else guest.birthday::int<=${fld:f_endday}::int end)

--是否参加过俱乐部
and (case when ${fld:f_participate} is null then 1=1 else guest.participate = ${fld:f_participate} end)
--是否曾是本俱乐部会员
and (case when ${fld:f_ismember} is null then 1=1 else guest.ismember = ${fld:f_ismember} end)
--健身目的
and (case when ${fld:f_purpose} is null then 1=1 else guest.purpose = ${fld:f_purpose} end)
--个人爱好
and (case when ${fld:f_personalhobbit} is null then 1=1 else guest.personalhobbit = ${fld:f_personalhobbit} end)
--婚姻情况
and (case when ${fld:f_marriage} is null then 1=1 else guest.marriage = ${fld:f_marriage} end)
--子女情况
and (case when ${fld:f_children} is null then 1=1 else guest.children = ${fld:f_children} end)

--0会员1资源
and (case when ${fld:f_custtype} is null then 1=1 
		when ${fld:f_custtype}='0' then 1=1
		when ${fld:f_custtype}='1' then 1=2 
	end) 
--录入日期
and (case when ${fld:startdate} is null then 1=1 else guest.created >= ${fld:startdate} end)
--录入日期
and (case when ${fld:enddate} is null then 1=1 else guest.created <= ${fld:enddate} end)
...
and (case when ${fld:custtype} is null then 1=1 else '1'=${fld:custtype} end)
and (case when ${fld:specialtype} is null then 1=1 else '1'=${fld:specialtype} end)
--回访状态
and (case when ${fld:f_returntype} is null then 1=1 
	when ${fld:f_returntype}='0' then 
	exists(
...
and (case when ${fld:f_sex} is null then 1=1 else exists(
				...
				and cust.sex=${fld:f_sex}
			) end)
--会籍
and (case when ${fld:f_mc} is null then 1=1 else exists(
				select 1 from cc_customer where org_id = ${def:org}
				and cust.mc=${fld:f_mc}
			) end)
--私教
and (case when ${fld:f_pt} is null then 1=1 else exists(
				select 1 from cc_customer where org_id = ${def:org}
				and cust.pt=${fld:f_pt}
			) end)
--沟通阶段
and (case when ${fld:f_stage} is null then 1=1 else exists(
...
--未续费原因
and (case when ${fld:f_leave} is null then 1=1 else exists(
				select 1 from cc_customer where org_id = ${def:org}
				and cust.leave=${fld:f_leave}
			) end)
--年龄段
and (case when ${fld:f_age} is null then 1=1 else exists(
				select 1 from cc_customer where org_id = ${def:org}
				and cust.age=${fld:f_age}
			) end)	
--通话次数
and (case when ${fld:f_calltimes} is null then 1=1 else exists(
				select 1 from cc_comm where cust_type=1
				and (select count(customercode)  from cc_comm where customercode=cust.code)<${fld:f_calltimes}
			) end)	
--未跟进天数
and (case when ${fld:f_unfollow} is null then 1=1 else exists(
				select 1 from cc_comm where cust_type=1
...
--生日
and (case when ${fld:f_startmonth} is null then 1=1 else cust.birth::int>=${fld:f_startmonth}::int end)
and (case when ${fld:f_startday} is null then 1=1 else cust.birthday::int>=${fld:f_startday}::int end)
and (case when ${fld:f_endmonth} is null then 1=1 else cust.birth::int<=${fld:f_endmonth}::int end)
and (case when ${fld:f_endday} is null then 1=1 else cust.birthday::int<=${fld:f_endday}::int end)

--健身目的
and (case when ${fld:f_purpose} is null then 1=1 else cust.purpose = ${fld:f_purpose} end)
--个人爱好
and (case when ${fld:f_personalhobbit} is null then 1=1 else cust.personalhobbit = ${fld:f_personalhobbit} end)
--婚姻情况
and (case when ${fld:f_marriage} is null then 1=1 else cust.marriage = ${fld:f_marriage} end)
--子女情况
and (case when ${fld:f_children} is null then 1=1 else cust.children = ${fld:f_children} end)

--0会员1资源
and (case when ${fld:f_custtype} is null then 1=1 
		when ${fld:f_custtype}='0' then 1=2
		when ${fld:f_custtype}='1' then 1=1 
	end) 
--录入日期
and (case when ${fld:startdate} is null then 1=1 else cust.created >= ${fld:startdate} end)
--录入日期
and (case when ${fld:enddate} is null then 1=1 else cust.created <= ${fld:enddate} end)
...
and (case when ${fld:custtype} is null then 1=1 else '1'=${fld:custtype} end)
and (case when ${fld:specialtype} is null then 1=1 else '2'=${fld:specialtype} end)
--回访状态
and (case when ${fld:f_returntype} is null then 1=1 
	when ${fld:f_returntype}='0' then 
	exists(
