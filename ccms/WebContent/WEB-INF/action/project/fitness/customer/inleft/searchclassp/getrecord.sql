select
	...
	(select classroom_name from cc_classroom where code = cl.classroomcode and org_id = ${def:org}) as classroom_name,
	(select name from hr_staff where userlogin = cl.teacherid and org_id = ${def:org}) as staff_name
...
and cp.org_id = ${def:org} and cp.status = 1