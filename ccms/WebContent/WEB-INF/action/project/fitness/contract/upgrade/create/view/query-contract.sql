select 
	...
	,(
		select name from cc_cardtype where code = get_arr_value(t.relatedetail, 14) and org_id = ${def:org}
	) as oldcardtypename
	,get_arr_value(t.relatedetail, 25) as oldenddate
	,get_arr_value(t.relatedetail, 26) as oldnowcount
	,(select name from hr_staff where userlogin = get_arr_value(t.relatedetail, 27)) as oldmcname
	,(select name from hr_staff where userlogin = get_arr_value(t.relatedetail, 28)) as newmcname
