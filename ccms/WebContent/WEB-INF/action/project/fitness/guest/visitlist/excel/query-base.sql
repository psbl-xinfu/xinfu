...
from (
	...
	select 	
		...
		(case when (select count(1) from cc_guest_visit where guestcode = g.code and org_id = ${def:org})>0 then '已来访' else 
	...
	where (case when ${fld:daochu_cust_all} is null then (p.preparedate >= ${fld:daochu_start_date} and p.preparedate <= ${fld:daochu_end_date}) else 1=1 end)
		and g.org_id = ${def:org}
		and (case when ${fld:daochu_status} is null then 1=1 
			when ${fld:daochu_status}='6'
			then exists(
			...
			else p.status = ${fld:daochu_status} end)
		and (case when ${fld:daochu_cust_all} is null then 1=1 else (g.code like concat('%', ${fld:daochu_cust_all}, '%') or g.name like concat('%', ${fld:daochu_cust_all}, '%') or g.mobile like concat('%', ${fld:daochu_cust_all}, '%')) end)
		and (case when ${fld:daochu_mc} is null then 1=1 else p.createdby = ${fld:daochu_mc} end)
	...
	where (v.preparecode is null or v.preparecode = '') and g.org_id = ${def:org}
		and (case when ${fld:daochu_cust_all} is null then (v.visitdate >= ${fld:daochu_start_date} and v.visitdate <= ${fld:daochu_end_date}) else 1=1 end)
		and (case when ${fld:daochu_mc} is null then 1=1 else v.mc = ${fld:daochu_mc} end) 
		and (case when ${fld:daochu_status} is null then true when ${fld:daochu_status} = '4' then true else false end)
		and (case when ${fld:daochu_cust_all} is null then 1=1 else (g.code like concat('%', ${fld:daochu_cust_all}, '%') or g.name like concat('%', ${fld:daochu_cust_all}, '%') or g.mobile like concat('%', ${fld:daochu_cust_all}, '%')) end)
		and (case when exists(select 1 from hr_staff_skill hss inner join hr_skill hs on hss.skill_id = hs.skill_id 
					where (hs.org_id = ${def:org} or exists(select 1 from hr_staff_org so where hs.org_id = so.org_id and userlogin = '${def:user}'))
					and hss.userlogin = '${def:user}' and hs.data_limit = 1)
	...
	and (case when ${fld:daochu_cust_all} is null then 1=1 else (g.code like concat('%', ${fld:daochu_cust_all}, '%') or g.name like concat('%', ${fld:daochu_cust_all}, '%') or g.mobile like concat('%', ${fld:daochu_cust_all}, '%')) end)
	where (case when ${fld:daochu_cust_all} is null then (c.createdate >= ${fld:daochu_start_date} and c.createdate <= ${fld:daochu_end_date}) else 1=1 end)
