...
SELECT
	...
	config.category = 'OpeCategory' AND config.param_text = '修改已收款合同' AND config.status = 1 and 
	...
	,concat(
		${fld:vc_contractcode}, ';', ${fld:old_adate}, ';'||${fld:c_adate}
		, ';', ${fld:f_normalmoney}, ';', ${fld:f_factmoney}
		, ';', ${fld:vc_cardcode}, ';', ${fld:vc_cardcode}
		, ';', COALESCE(${fld:old_remark},''), ';', COALESCE(${fld:vc_remark},'')
	)
	,0
	,0
	,0
	...
	,'${def:user}'
FROM cc_contract t 
WHERE t.code = ${fld:vc_contractcode} and t.org_id = ${def:org}
