...
SELECT 
	t.vc_code	-- 合同号
	,t.vc_name	-- 姓名
	,CASE WHEN t.i_contracttype = 1 OR t.i_contracttype = 2 OR t.i_type = 6 THEN 'false' 	/** 升级合同 */
		WHEN t.i_contracttype = 3 THEN 'false' 	/** 还款合同 */
		WHEN t.i_type = 5 THEN 'false' 	/** 定金合同 */
		WHEN t.i_type = 0 THEN 'true' 	/** 办卡合同 */
		WHEN t.i_type = 7 OR t.i_type = 9 OR t.i_type = 11 THEN 'false' 	/** 续卡合同 */
		WHEN t.i_type = 10 THEN 'false' 	/** 转卡合同 */
		WHEN t.i_type = 4 THEN 'false' 	/** 退卡合同 */
		WHEN t.i_type = 2 THEN 'false' 	/** 私教合同 */ 
		WHEN t.i_type = 1 OR t.i_type = 12 THEN 'false'/** 租柜合同 */ ELSE '' END AS iscardshow	-- 卡号是否需要显示
	...
	,r.f_normalmoney	-- 应收
	,r.f_factmoney--实收
	,(r.f_normalmoney - r.f_factmoney) AS f_leftmoney	-- 尚欠
	,t.i_discounttype	-- 折扣
	...
	,f.f_pos
	,(select name from hr_staff where hr_staff.userlogin=f.vc_auser ) as vc_auser --收款人
    ,f.c_atime
    ,f.vc_code as pintiao_hao
    ,(SELECT tenantry_name FROM t_tenantry WHERE tenantry_id = ${def:tenantry}) as vc_club
    ,(SELECT vc_content as wish FROM e_cnfg WHERE	vc_category = 'Wish') as vc_remark
    ,(
		select string_agg(vc_content||'：'||get_arr_value(f.pay_detail,rowno),'<br/>') 
		from paytype 
		where 0.00 != COALESCE(get_arr_value(f.pay_detail,rowno),'0.00')::numeric(10,2) 
	) as paydetail
FROM e_contract t 
INNER JOIN e_contractrow r ON t.vc_code = r.vc_code 
LEFT JOIN hr_staff u ON t.vc_iuser = u.userlogin 
INNER JOIN e_finance f on t.vc_billcode=f.vc_code
