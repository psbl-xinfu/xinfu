...
from (
	...
	and exists(
	 		...
	 		and (case when ${fld:daochu_skill_name} is null then 1=1 else fk.userlogin=${fld:daochu_skill_name} end)
	 	)
	group by p.ptid 
) as t1 
full join (
...
full join(
	...
	and (case when ${fld:daochu_start_date} is null then 1=1 else c.createdate>=${fld:daochu_start_date} end)
	and (case when ${fld:daochu_end_date} is null then 1=1 else c.createdate<=${fld:daochu_end_date} end)
 	and (case when ${fld:daochu_skill_name} is null then 1=1 else get_arr_value(c.relatedetail,8)=${fld:daochu_skill_name} end)
 	and exists(
			select 1 from hr_skill k 
			inner join hr_staff_skill fk on k.skill_id = fk.skill_id 
