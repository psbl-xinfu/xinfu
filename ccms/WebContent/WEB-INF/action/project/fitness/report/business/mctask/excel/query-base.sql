...
(select 
	...
	(select count(1) from cc_guest guest
		...
		and (case when ${fld:daochu_start_date} is null then 1=1 else guest.created::date>=${fld:daochu_start_date} end)
		and (case when ${fld:daochu_end_date} is null then 1=1 else guest.created::date<=${fld:daochu_end_date} end)
	 	and (case when ${fld:daochu_skill_name} is null then 1=1 else guest.mc=${fld:daochu_skill_name} end)
	) as guestnum,
	(select count(1) from cc_customer cust
		...
		and (case when ${fld:daochu_start_date} is null then 1=1 else cust.created::date>=${fld:daochu_start_date} end)
		and (case when ${fld:daochu_end_date} is null then 1=1 else cust.created::date<=${fld:daochu_end_date} end)
	 	and (case when ${fld:daochu_skill_name} is null then 1=1 else cust.mc=${fld:daochu_skill_name} end)
	) as custnum
FROM hr_staff s 
inner join (select user_id from hr_staff_skill 
	inner join (
	select skill_id from hr_skill where org_id = ${def:org} and
		skill_scope = '2') skill on skill.skill_id = hr_staff_skill.skill_id
	) hs on hs.user_id = s.user_id
WHERE s.is_member = 0 AND s.status = 1
and (case when ${fld:daochu_skill_name} is null then 1=1 else s.userlogin=${fld:daochu_skill_name} end) 
...
FULL JOIN(
	...
	and exists(
		...
		where k.skill_scope = '2'  and fk.userlogin = ts.userlogin and k.org_id = ts.org_id
	)
...
FULL JOIN(
	...
	and (case when ${fld:daochu_start_date} is null then 1=1 else g.created::date>=${fld:daochu_start_date} end)
	and (case when ${fld:daochu_end_date} is null then 1=1 else g.created::date<=${fld:daochu_end_date} end)
 	and (case when ${fld:daochu_skill_name} is null then 1=1 else g.initmc=${fld:daochu_skill_name} end)
	and exists(
		select 1 from hr_skill k 
		inner join hr_staff_skill fk on k.skill_id = fk.skill_id 
		where k.skill_scope = '2' and fk.userlogin = g.initmc and k.org_id = g.org_id
	) GROUP BY g.initmc
...
FULL JOIN(
	...
	and (case when ${fld:daochu_start_date} is null then 1=1 else c.created::date>=${fld:daochu_start_date} end)
	and (case when ${fld:daochu_end_date} is null then 1=1 else c.created::date<=${fld:daochu_end_date} end)
 	and (case when ${fld:daochu_skill_name} is null then 1=1 else c.createdby=${fld:daochu_skill_name} end)
	and exists(
		select 1 from hr_skill k 
		inner join hr_staff_skill fk on k.skill_id = fk.skill_id 
		where k.skill_scope = '2' and fk.userlogin = c.createdby and k.org_id = c.org_id
...
FULL JOIN(
	...
	and (case when ${fld:daochu_start_date} is null then 1=1 else gp.created::date>=${fld:daochu_start_date} end)
	and (case when ${fld:daochu_end_date} is null then 1=1 else gp.created::date<=${fld:daochu_end_date} end)
 	and (case when ${fld:daochu_skill_name} is null then 1=1 else gp.createdby=${fld:daochu_skill_name} end)
	and exists(
		select 1 from hr_skill k 
		inner join hr_staff_skill fk on k.skill_id = fk.skill_id 
		where k.skill_scope = '2' and fk.userlogin = gp.createdby and k.org_id = gp.org_id
...
FULL JOIN(
	...
	and (case when ${fld:daochu_start_date} is null then 1=1 else gv.created::date>=${fld:daochu_start_date} end)
	and (case when ${fld:daochu_end_date} is null then 1=1 else gv.created::date<=${fld:daochu_end_date} end)
 	and (case when ${fld:daochu_skill_name} is null then 1=1 else gv.mc=${fld:daochu_skill_name} end)
	and exists(
		select 1 from hr_skill k 
		inner join hr_staff_skill fk on k.skill_id = fk.skill_id 
		where k.skill_scope = '2' and fk.userlogin = gv.mc and k.org_id = gv.org_id
	) GROUP BY gv.mc
...
FULL JOIN(
	...
	and (case when ${fld:daochu_start_date} is null then 1=1 else c.createdate::date>=${fld:daochu_start_date} end)
	and (case when ${fld:daochu_end_date} is null then 1=1 else c.createdate::date<=${fld:daochu_end_date} end)
 	and (case when ${fld:daochu_skill_name} is null then 1=1 else c.salemember=${fld:daochu_skill_name} end)
	and exists(
 			select 1 from hr_skill k 
 			inner join hr_staff_skill fk on k.skill_id = fk.skill_id 
 			where k.skill_scope = '2' and fk.userlogin = c.salemember and k.org_id = c.org_id
 		)
	...
	and (case when ${fld:daochu_start_date} is null then 1=1 else c.createdate::date>=${fld:daochu_start_date} end)
	and (case when ${fld:daochu_end_date} is null then 1=1 else c.createdate::date<=${fld:daochu_end_date} end)
 	and (case when ${fld:daochu_skill_name} is null then 1=1 else c.salemember1=${fld:daochu_skill_name} end)
	and exists(
 			select 1 from hr_skill k 
 			inner join hr_staff_skill fk on k.skill_id = fk.skill_id 
 			where k.skill_scope = '2' and fk.userlogin = c.salemember1 and k.org_id = c.org_id
 		)
