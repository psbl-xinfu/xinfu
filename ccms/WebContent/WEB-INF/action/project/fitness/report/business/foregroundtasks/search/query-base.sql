...
(SELECT
	...
	(select count(1) from cc_feedback_follow f 
		...
		and (case when ${fld:start_date} is null then 1=1 else f.created::date>=${fld:start_date} end)
		and (case when ${fld:end_date} is null then 1=1 else f.created::date<=${fld:end_date} end)
 		and (case when ${fld:skill_name} is null then 1=1 else f.followstaff=${fld:skill_name} end)
	) as kefucount
from hr_staff staff
...
FULL JOIN(
	...
	where rl.status != 0 and rl.org_id = ${def:org} and (rl.datatype=0 or rl.datatype=1)
		and (case when ${fld:start_date} is null then 1=1 else rl.created::date>=${fld:start_date} end)
		and (case when ${fld:end_date} is null then 1=1 else rl.created::date<=${fld:end_date} end)
 		and (case when ${fld:skill_name} is null then 1=1 else rl.followby=${fld:skill_name} end)
	and exists(
		select 1 from hr_skill k 
		inner join hr_staff_skill fk on k.skill_id = fk.skill_id 
