...
(SELECT
	...
	and (case when ${fld:daochu_start_date} is null then 1=1 else p.created::date>=${fld:daochu_start_date} end)
	and (case when ${fld:daochu_end_date} is null then 1=1 else p.created::date<=${fld:daochu_end_date} end)) as custnum,
	...
	and not exists(
		...
		and (case when ${fld:daochu_start_date} is null then 1=1 else p.created::date>=${fld:daochu_start_date} end)
		and (case when ${fld:daochu_end_date} is null then 1=1 else p.created::date<=${fld:daochu_end_date} end)
	)) as guestnum
from hr_staff staff
...
and exists(
	...
	where k.skill_scope = '1'  and fk.userlogin =staff.userlogin and k.org_id = staff.org_id
)
) as t1
FULL JOIN(
	select 
	...
	and exists(
		select 1 from hr_skill k 
		inner join hr_staff_skill fk on k.skill_id = fk.skill_id 
		where k.skill_scope = '1'  and fk.userlogin = ts.userlogin and k.org_id = ts.org_id
	)
...
FULL JOIN(
	...
	and exists(
		select 1 from hr_skill k 
		inner join hr_staff_skill fk on k.skill_id = fk.skill_id 
		where k.skill_scope = '1' and fk.userlogin = c.createdby and k.org_id = c.org_id
...
FULL JOIN(
	...
	where tr.org_id = ${def:org} and tr.status = 1
	and (case when ${fld:daochu_start_date} is null then 1=1 else tr.created::date>=${fld:daochu_start_date} end)
	and (case when ${fld:daochu_end_date} is null then 1=1 else tr.created::date<=${fld:daochu_end_date} end)
 	and (case when ${fld:daochu_skill_name} is null then 1=1 else tr.createdby=${fld:daochu_skill_name} end)
	and exists(
		select 1 from hr_skill k 
		inner join hr_staff_skill fk on k.skill_id = fk.skill_id 
		where k.skill_scope = '1' and fk.userlogin = tr.createdby and k.org_id = tr.org_id
...
full join (
	...
	and (case when ${fld:daochu_start_date} is null then 1=1 else pr.created::date>=${fld:daochu_start_date} end)
	and (case when ${fld:daochu_end_date} is null then 1=1 else pr.created::date<=${fld:daochu_end_date} end)
 	and (case when ${fld:daochu_skill_name} is null then 1=1 else pr.ptid=${fld:daochu_skill_name} end)
	and exists(
 			select 1 from hr_skill k 
 			inner join hr_staff_skill fk on k.skill_id = fk.skill_id 
...
full join (
	...
	and (case when ${fld:daochu_start_date} is null then 1=1 else p.created::date>=${fld:daochu_start_date} end)
	and (case when ${fld:daochu_end_date} is null then 1=1 else p.created::date<=${fld:daochu_end_date} end)
 	and (case when ${fld:daochu_skill_name} is null then 1=1 else p.ptid=${fld:daochu_skill_name} end)
	and exists(
	 		select 1 from hr_skill k 
	 		inner join hr_staff_skill fk on k.skill_id = fk.skill_id 
...
FULL JOIN(
	...
	and (case when ${fld:daochu_start_date} is null then 1=1 else c.createdate>=${fld:daochu_start_date} end)
	and (case when ${fld:daochu_end_date} is null then 1=1 else c.createdate<=${fld:daochu_end_date} end)
