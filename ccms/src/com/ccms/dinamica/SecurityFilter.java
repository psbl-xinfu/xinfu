    ...
    */
...

    {
        ...
        try
        {
    		...
    		String prefix_url = (uri.indexOf("?")>0)?uri.substring(0, uri.indexOf("?")):uri;
        	if (user==null)
			{
        		if(!excludeUrls.containsKey(prefix_url)){
    				if ( _debug )
			...
			//is protected?
			if (protectedRes.containsKey(uri) && !excludeUrls.containsKey(prefix_url))
			{
